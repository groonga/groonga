...
grn_pat_prefix_search(grn_ctx *ctx,
                      ...
                      grn_hash *h);
GRN_API grn_rc
grn_pat_suffix_search(grn_ctx *ctx,
                      grn_pat *pat,
                      const void *key,
                      unsigned int key_size,
                      grn_hash *h);
...
GRN_API int
grn_pat_cursor_get_key_value(grn_ctx *ctx,
                             grn_pat_cursor *c,
                             void **key,
                             unsigned int *key_size,
                             void **value);
  ...
  do {                                                                         \
    ...
    if (_sc) {                                                                 \
      ...
      while ((id = grn_pat_cursor_next(ctx, _sc))) {                           \
        grn_pat_cursor_get_key_value(ctx,                                      \
                                     _sc,                                      \
                                     (void **)(key),                           \
                                     (key_size),                               \
                                     (void **)(value));                        \
...
#define GRN_PAT_EACH_BEGIN(ctx, pat, cursur, id, key, key_size, value)         \
  do {                                                                         \
    grn_pat_cursor *cursur = NULL;                                             \
    if (pat && grn_pat_size(ctx, pat) > 0) {                                   \
      cursur = grn_pat_cursor_open(ctx, pat, NULL, 0, NULL, 0, 0, -1, 0);      \
    }                                                                          \
    if (cursur) {                                                              \
      grn_id id;                                                               \
