...
.. list-table::
   ...
   * - ``COMPRESS_FILTER_SHUFFLE``
     - .. versionadded:: 13.0.8
       ...
       このフラグは、カラムの圧縮前にベクターカラム内の各要素の0バイト目だけのデータを集めて連続で配置します。
       その後、同様に1バイト目のデータだけを集めて連続でで配置するということをすべてのバイトに対して繰り返します。
       こうすることで、 ``COMPRESS_LZ4`` や ``COMPRESS_ZSTD`` と組み合わせて使った際に圧縮率を向上できることがあります。
       ただし、データによっては圧縮率が変わらなかったり、逆に下がったりすることがあります。

       このフラグで指定するフィルターは以下のように動作します。
       ...
       どちらも値がバラバラになっています。こういったデータはシャッフルしても圧縮効率が上がらないため、このフラグを使うには
       不向きなデータです。
