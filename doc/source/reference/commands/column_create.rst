...
.. list-table::
   ...
   * - ``COMPRESS_ZSTD``
     ...
     * - ``COMPRESS_FILTER_SHUFFLE``
       ...
       圧縮対象のベクターカラムの各要素を1バイト単位で区切ってまとめるフィルターです。
       ...
       ベクターカラムの各要素を1バイト単位で区切ってまとめるとは、以下のようなことです。

       例えば、[f0, f1, f2, f3]という4バイトの値を保持しているベクターカラムがあったとします。
       これらの、0バイト目同士、1バイト目同士、...3バイト目同士をまとめます。
       ...
       上記のように各バイトごとにまとめると同じ値が存在しやすくなります。
       zstdやLZ4は、同じ値が連続していると圧縮率が高くなりやすい圧縮アルゴリズムです。
       上記の操作で同じ値が連続しやすくなるようにデータを変形し、圧縮率の向上を狙っています。
