...
.. list-table::
   ...
   * - ``COMPRESS_FILTER_SHUFFLE``
     - .. versionadded:: 13.0.8
       ...
       例1. ``UInt32: [1, 258, 515]`` というベクターカラムの場合は以下のようになります。

       .. code-block::

          UInt32: [1, 258, 515] -> バイト列（リトルエンディアン）:["\x01\x00", "\x02\x01", \x03\x02"]
          -> シャッフル: "\x01\x02\x03" （1バイト目をまとめたもの）+ "\x00\x01\x02"（2バイト目をまとめたもの）

       例2. ``UInt32: [1, 2, 3]`` というベクターカラムの場合は以下のようになります。

       .. code-block::

          UInt32: [1, 2, 3] -> バイト列（リトルエンディアン）:["\x01\x00", "\x02\x00", \x03\x00"]
          シャッフル: "\x01\x02\x03" （1バイト目をまとめたもの）+ "\x00\x00\x00"（2バイト目をまとめたもの）

       例1では、シャッフル後に同じ値が連続していません。「1バイト目をまとめたもの」も「2バイト目をまとめたもの」も
       どちらも値がバラバラになっています。こういったデータはシャッフルしても圧縮効率が上がらないため、このフィルターを使うには
       不向きなデータです。

       例2では、「2バイト目をまとめたもの」がすべて0になり、同じ値が連続するため ``Zstandard`` や ``LZ4`` で圧縮したときに
       圧縮効率が上がります。

       このように、このフラグはシャッフル後に同じ値が連続するようなデータのときに効果を発揮します。
       浮動小数点数のデータであれば、符号が同じであったり、桁数が同じデータです。
       （浮動小数点数の内部表現では、符号や桁数を表現するデータは同じ場所に配置されているからです。
       符号なら最上位ビットですし、桁数なら23ビット目から30ビット目に配置されます。
       これらが同じ値であれば、バイト単位でにまとめた際に同じ値が連続しやすくなります。）

       逆に例1のようにシャッフル後に同じ値が連続しないデータの場合は効果が薄いです。
       また、ベクターカラムの要素数が少ない場合も効果的ではありません。(例1, 例2では、わかりやすさのために要素数の少ないデータを
       例にしていますが、もっと要素数の多いデータに対して使用することを想定しているフィルターです。)
       ...
       単独でも効果がありますが、 ``COMPRESS_FILTER_TRUNCATE_PRECISION_1BYTE`` と ``COMPRESS_FILTER_TRUNCATE_PRECISION_2BYTES``
       と組み合わせると、より高い効果が期待できます。
       ただし、 ``COMPRESS_FILTER_TRUNCATE_PRECISION_1BYTE`` と ``COMPRESS_FILTER_TRUNCATE_PRECISION_2BYTES`` は、``Float/Float32`` 型
       の値にしか使えないことに注意してください。
   ...
   * - ``COMPRESS_FILTER_BYTE_DELTA``
     - .. versionadded:: 13.0.8
       ...
       1バイトのデータ( ``Int8/UInt8 `` )の場合は、 ``COMPRESS_FILTER_SHUFFLE`` は使いません。(使っても効果がありません。)
       このフィルターを使うことで、 ``COMPRESS_LZ4`` や ``COMPRESS_ZSTD`` の圧縮率の向上が期待できます。
       ただし、データによっては圧縮率が変わらなかったり、逆に下がったりすることがあります。

       このフィルターは、例えば、 UInt32: [1, 2, 3, 4, 5] のように一定のペースで値が増加していくデータや UInt32: [5,5,5,5,5] のように同じ値が連続しているデータ
       に適したフィルターです。
       UInt32: [1, 2, 3, 4, 5]のデータは、各要素間の差分を計算すると、差分データ: [1,1,1,1,1]になり、UInt32: [5,5,5,5,5]のデータの差分を計算すると
       差分データ: [5,0,0,0,0]になり、同じ値が連続するため ``Zstandard`` や ``LZ4`` で圧縮したときに圧縮効率が上がります。
       また、 UInt32: [10000,10001,10002] のように大きな値であっても差分を計算すると、差分データ: [10000, 1, 1]と小さい値になります。
       小さな値は少ないバイト列で表現することで圧縮効果が高くなることがあります。

       一方で、以下のように差分を計算しても同じ値が連続しなかったり、差分自体が大きい値になる場合にはこのフィルターは不向きです。
       UInt32: [47, 46580, 4592401, 45266770] -> 差分データ:[47, 46533, 4545821, 40674369]
       ...
       .. code-block::
          ...
          UInt32: [4526677, 4592401, 4658217, 4723879] -> バイト列（リトルエンディアン）：["\x55\x12\x45", "\x11\x13\x46", "\x29\x14\x47", "\xA7\x14\x48"]
          シャッフル："\x55\x11\x29\xA7"（1バイト目をまとめたもの） + "\x12\x13\x14\x14"（2バイト目をまとめたもの） + "\x45\x46\x47\x48"（3バイト目をまとめたもの）

       となります。
       ここで、 ``COMPRESS_FILTER_BYTE_DELTA`` を適用すると
       シャッフル後の2バイトえおまとめたものと3バイト目をまとめたものの差分データは、それぞれ ["\x12", "\x1", "\x1", "\x0"], ["\x45", "\x1", "\x1", "\x1"]となり
       差分の値が小さくなったり、同じ値が連続したりするので、 ``Zstandard`` や ``LZ4`` で圧縮したときに圧縮効率が上がります。

       このように、差分を計算しただけでは圧縮率の向上が見込めないデータでも、 ``COMPRESS_FILTER_SHUFFLE`` と合わせて使うことで
       圧縮率の向上を見込めるケースがあります。
   ...
   * - ``COMPRESS_FILTER_TRUNCATE_PRECISION_1BYTE``
     - .. versionadded:: 13.0.8
       ...
       このフィルターは、浮動小数点数の精度を1バイト落とすので、リトルエンディアンの場合は最上位バイト
       ビックエンディアンの場合は最下位バイトをすべて0にします。
       上記の例はリトルエンディアンなので、最上位バイトを0にして以下のようにします。

       .. code-block::

          [ "\x3F\xA0\x00\x00", "\x40\x6A\xE1\x00", "\x40\x91\x99\x00"]
       ...
       .. code-block::
          ...
          "\x3F\x40\x40"（1バイト目をまとめたもの） + "\xA0\x6A\x91"（2バイト目をまとめたもの） + "\x00\xE1\x99"（3バイト目をまとめたもの） + "\x00\x00\x00"（4バイト目をまとめたもの）

       上記のように、浮動小数点の精度を1バイト落とすことで、 ``COMPRESS_FILTER_SHUFFLE`` した時に0が連続するデータを作ることができます。
       上記の例では、「4バイト目をまとめたもの」のことです。
