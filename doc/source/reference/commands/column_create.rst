...
.. list-table::
   ...
   * - ``COMPRESS_FILTER_BYTE_DELTA``
     - .. versionadded:: 13.0.8
       ...
       2バイト目同士: [f0[2], (f0[2] - f1[2]), (f1[2] - f2[2]), (f2[2] - f3[2])]
       3バイト目同士: [f0[3], (f0[3] - f1[3]), (f1[3] - f2[3]), (f2[3] - f3[3])]

       nバイト毎のデータの差分を計算すると、値が小さくなり、値が0になりやすくなるため、 ``COMPRESS_FILTER_SHUFFLE``
       単独で使うよりも高い圧縮効果を期待できます。
