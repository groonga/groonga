...
.. list-table::
   ...
   * - ``COMPRESS_FILTER_SHUFFLE``
     - .. versionadded:: 13.0.8
       ...
       このフラグは、上記のByte 0の値をまとめて、その後Byte 1のデータをまとめ以下のようなデータを作るフラグです。
       ...
       まず、説明のため ``UInt32``:[1,2,3]を以下のようなリトルエンディアンのバイト列表現にします。
       ...
       ``UInt32``:[1,2,3]というデータだと、シャッフル後にByte 0のデータがすべて0になり、同じ値が連続することになります。
