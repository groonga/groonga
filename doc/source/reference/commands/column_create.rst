...
.. list-table::
   ...
   * - ``COMPRESS_FILTER_SHUFFLE``
     - .. versionadded:: 13.0.8
       ...
       ただし、データによっては効果がないこともあります。

       バイト間の差分を計算するというのは、以下のようなことです。
       ``COMPRESS_FILTER_SHUFFLE`` と組み合わせて使う場合を例にします。
       ``COMPRESS_FILTER_SHUFFLE`` を適用すると、前述の通り以下のようなデータができます。

       0バイト目同士: [f0[0], f1[0], f2[0], f3[0]]
       1バイト目同士: [f0[1], f1[1], f2[1], f3[1]]
       2バイト目同士: [f0[2], f1[2], f2[2], f3[2]]
       3バイト目同士: [f0[3], f1[3], f2[3], f3[3]]

       これらのnバイト目同士の各要素間の差分を計算するので、 ``COMPRESS_FILTER_BYTE_DELTA`` を適用すると
       最終的には以下のようなデータができます。
