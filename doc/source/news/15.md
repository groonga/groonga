...
This problem may occurs when we use `*NP`, `*NPP`, or `*ONP` with `MAX_ELEMENT_INTERVAL` as below.
...
- [`*ONPP` in query syntax] {ref}`query-syntax-ordered-near-phrase-search-condition`
- [`*ONPP` in script syntax] {ref}`script-syntax-ordered-near-phrase-search-operator`
