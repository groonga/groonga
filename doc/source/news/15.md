...
We can execute parallel vectorization in {doc}`/reference/functions/language_model_vectorize`, {doc}`/reference/tokenizers/token_language_model_knn`, and {doc}`/reference/functions/language_model_vectorize`.

However, if we set `--n-default-workers=-1`, Groonga execute in parallel with the threads of at most the number of CPU cores.
