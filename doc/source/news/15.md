...
Therefore, `ghrndb check` is not practical on low-memory environment because of it takes a long time to complete check.

We can complate `grndb check` within a realistic time even if we use low-memory environment by this modification.
