...
Previously, TokenMecab internally treated MeCab’s `-Owakati` mode output as
space-delimited tokens, causing user-defined dictionary entries containing
spaces (e.g., "search engine") to be split into separate tokens like "search"
and "engine". This release updates TokenMecab to use MeCab’s `-F%m\n`
newline-delimited output, ensuring that entries with embedded spaces remain as
single tokens.
