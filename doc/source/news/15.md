...
#### {ref}`offline-index-construction`: Fixed a bug that Groonga may ingnore normalizer options of target index when offline index construction execute parallel

If happend when:

- Execute offline index construction with parallel
- Use Normalizer/Tokenizer/TokenFilter options

However, if we use NormalizerTable, Groonga occurs error.
