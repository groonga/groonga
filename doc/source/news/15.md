...
## Release 15.2.2 - 2025-12-24

### Improvements

#### Bundled OpenZL(experimental)

> This feature is still experimental and unstable.
> We must not use this feature in production.
...
Because we don't implement for feature of compress and decompress.

#### Added support JSON type(experimental)

> This feature is still experimental and unstable.
> We must not use this feature in production.

We can `load` and `dump` against some data types.
However, we don't implement other futures yet.

### Fixes

#### Fixed a bug that Groonga couldn't find records that added after index construction when `centroid_column` was used
