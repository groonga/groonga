...
#### Fixed a bug that occurs overflow in the key of `TABLE_PAT_KEY` tables.

This issue is found in PGroonga by [pgroonga/pgroonga#910](https://github.com/pgroonga/pgroonga/issues/901).
If you execute offline index construction against the column has large size value(the value such as the size of token is 4096 bytes after tokenize), the index may be broken.
