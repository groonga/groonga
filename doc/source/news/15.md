...
#### [{doc}`/install/cmake`] Remove a needless option. [GH-2479][Patched by Tsutomu Katsube]

#### [{doc}`/reference/commands/table_create`] Fix a bug that Groonga dropped `KEY_LARGE` flag from `TABLE_HASH_KEY` after we execute a `truncate` command.
