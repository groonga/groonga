...
#### [{doc}`/install/debian`] Added support for Debian 13

Debian 13 packages are now available.
You can install Groonga on Debian 13 using the standard package installation methods.
...
#### [{doc}`/reference/executables/grndb`] Fixed a bug that the database checked by `grndb check` command was not enought

Because `grndb check` command did not check the part of database files.

It's no problem with that tables and columns check by `grndb check` command.
