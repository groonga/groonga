...
In this release, we supported for `KEY_LARGE` flag for `TABLE_PAT_KEY`!
...
the behavior was inconsistent. The hyphen characters were not properly treated as symbols to be removed.

This release fixes this issue. Now when both options are enabled:

- Hyphen characters (-, ー, —, ―, etc.) are treated as symbols
- These characters are properly removed from the normalized text

For example:

```
normalize 'NormalizerNFKC("remove_symbol", true, "unify_hyphen_and_prolonged_sound_mark", true)' "090ー1234-5678"
[
  [
    0,
    1756363926.409565,
...
#### [{doc}`/reference/commands/table_create`] Added support for `KEY_LARGE` option for `TABLE_PAT_KEY`

You can now use the `KEY_LARGE` flag with `TABLE_PAT_KEY` tables to expand the maximum total key size from 4GiB to 1TiB,
similar to `TABLE_HASH_KEY` tables. This allows you to store more keys in total, not larger individual keys.

Previously, `KEY_LARGE` was only available for `TABLE_HASH_KEY`.
This limitation meant that users who needed patricia trie features couldn't store more than 4GiB of total key data.
With this enhancement, you can create patricia trie tables that can hold more keys:
...
#### [{doc}`/install/cmake`] Fixed incorrect CMake build and install commands [GH-2479][Patched by Tsutomu Katsube]

The documentation previously included an incorrect `-B` option in the `cmake --build` and `cmake --install` commands,
...
#### [{doc}`/reference/commands/table_create`] Fixed a bug where `KEY_LARGE` flag was lost after executing `truncate` command

Previously, when you executed the `truncate` command on a table with the `KEY_LARGE` flag,
the flag was incorrectly removed. This affected both `TABLE_HASH_KEY` and `TABLE_PAT_KEY` tables.

This bug caused truncated tables to lose their ability to store more than 4GiB of total key data,
potentially leading to unexpected failures when trying to load large amounts of data after truncation.

The issue has been fixed by correctly preserving table flags during the truncate operation now.
