...
Previously, {ref}`token-mecab` split user-defined dictionary entries containing
spaces (e.g., "search engine") into separate tokens ("search" and "engine").
This release fixes this issue, so entries with embedded spaces are now preserved
and handled as single tokens like "search engine".
