...
In this release, we supported NormalizerNFKC's `unify_iteration_mark` option to normalize Japanese iteration marks (ゝ/ゞ/ヽ/ヾ; 々/〻 as single-char repeats) and fixed installation failure on AlmaLinux 10.

### Improvements

#### [{doc}`/reference/executables/grndb`] Improved error handling for large files

Previously, `grndb` aborted when it encountered database files that exceeded filesystem stat limits.
This release makes those errors non-fatal. `grndb` now logs an error for each problematic file but continues processing to completion.
