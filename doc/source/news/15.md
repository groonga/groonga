...
Previously, `grndb` aborted when it encountered database files that exceeded
filesystem stat limits (such as files larger than 2GB on Windows 32-bit systems).
This release makes those errors non-fatal. When `grndb` encounters such files in
the Groonga database directory (the `db` file and its related `db.*` files), it
now logs an error for each problematic file but continues processing to
completion instead of aborting.

#### [{doc}`/reference/normalizers/normalizer_nfkc`] Added support for iteration marks with `unify_iteration_mark` option
...
A new `command_list` command has been added that returns a list of all available Groonga commands.
This is useful for discovering available functionality and for tools that need to introspect Groonga's capabilities.
...
Previously, the `TokenFilterStem` filter only worked with ASCII characters.
...
#### [{doc}`/install/almalinux`] Fixed installation failure on AlmaLinux 10 (GPG key)

Previously, installation could fail with a dnf GPG check error due to an outdated RPM GPG key in groonga-release.
We’ve removed the old key and now ship only the RSA4096 key, so installs work as expected now.

If you have already installed an older `groonga-release`, please upgrade it to the latest version to remove the old key:
