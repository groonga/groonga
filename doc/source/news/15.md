...
In this release, we can provide pacakges for Ubuntu again.

### Improvements

#### [{doc}`/reference/functions/language_model_vectorize`] Added support for parallel vectorization

If we set `--n-default-workers=-1`, Groonga execute in parallel with the threads of at most the number of CPU cores.
...
#### [{doc}`/install/ubuntu`] Provided pacakges again

We couldn't provide packages for Ubuntu in Groonga 15.1.8 caused by building failure in Lunchpad.
In this relase, we provide packages for Ubuntu again by resolving issue for building failure
