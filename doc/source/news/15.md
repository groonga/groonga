...
#### [Windows] Drop support for building with Visual Studio 2019

### Fixes

#### [{ref}`query-syntax-near-phrase-search-condition`][{ref}`script-syntax-near-phrase-search-operator`][{ref}`query-syntax-near-phrase-product-search-condition`][{ref}`script-syntax-near-phrase-product-search-operator`][{ref}`query-syntax-ordered-near-phrase-search-condition`][{ref}`script-syntax-ordered-near-phrase-search-operator`]: Fixed a bug that interval between phrases calculation

This problem may occurs the following conditions.

* Use `*NP`, `*NPP`, or `*ONP` with `MAX_ELEMENT_INTERVAL` as below.
...
Please refer to the following links about usages and syntaxes of `*NP`, `*NPP`, or `*ONPP`.
...
If this problem occur as follows.

* Groonga may return records shouldn't be a hit.
* Groonga may don't return records that should be hit.
