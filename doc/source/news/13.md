...
* Updated bundled mruby from 1.4.1 to 3.2.0.

* Update bundled xxhash from 0.8.0 to 0.8.2.
...
* [{doc}`/reference/commands/column_create`] Improved `column_create` flags with addition new flags `COLUMN_FILTER_SHUFFLE`, `COLUMN_FILTER_BYTE_DELTA`, `COMPRESS_FILTER_TRUNCATE_PRECISION_1BYTE`, and `COMPRESS_FILTER_TRUNCATE_PRECISION_2BYTES`.
  ...
  These flags aim at raise compression ratio of `COMPRESS_ZLIB`, `COMPRESS_LZ4`, and `COMPRESS_ZSTD` by filtering data using these flags before compress data.

  Even if we use these flags, These flags don't raise compression ratio depending on data.
  If enable these filter, process of column store and column reference additionally.
  Therefore, these processes are surely slow.

  The thing is use only filter that has effective.

  In addition, if we don't specify `COMPRESS_ZLIB`, `COMPRESS_LZ4`, and `COMPRESS_ZSTD`, these flags use [BloscLZ]( https://www.blosc.org/pages/blosc-in-depth/#blosc-as-a-meta-compressor ) as compress argorism.
  Therefore, we can small column size than the size of uncompressed column in almost case by enabling these flags.

  However, we are enough to just use `COMPRESS_ZLIB` or `COMPRESS_LZ4` or `COMPRESS_ZSTD` depending on data sometimes.
  Therefore, we suggest that we specify suitable flag according to data.

    These flags can use against only `COLUMN_VECTOR`.
  If we use `COLUMN_SCALAR`, these flags are ignored.
...
* [{doc}`/reference/commands/select`] Improved `select` arguments with addition new argument `--fuzzy_max_distance`.

  We useful this argument when we execute typo tolerance search.
  If the given query isn't matched, Groonga finds terms that have less than `--fuzzy_max_distance` edit distance from lexicon and searches with these found terms again as below.
...
* [{doc}`/reference/commands/select`] Improved `select` arguments with addition new argument `--fuzzy_max_expansions`.

  This argument set the max of the number of terms that close to edit distance to use search.

  For example, `This` and `That` exist in lexicon as terms that close to edit distance when we search `Thjs`.
  In this case, if we specify `--fuzzy_max_expansions 1`, Groonga search records using `This` after search with `Thjs`.
  Groonga does not search using `That`. Because we specify `--fuzzy_max_expansions 1`.
  If we specify `--fuzzy_max_expansions 2`, Groonga search records using `This` and `That` after search with `Thjs`.

  The default value of `--fuzzy_max_expansions` is `0`.
  If the given query isn't matched, Groonga finds all terms that close to edit distance from lexicon and searches with these found terms again when `--fuzzy_max_expansions` is `0`.

  If `--fuzzy_max_expansions` is `0`, Groonga may get more records related search keyword.
  However, if Groonga finds many terms that close to edit distance from lexicon, Groonga returns search results slowly.

  In this case, we can prevent that Groonga returns search results slowly by using `--fuzzy_max_expansions`.

* [{doc}`/reference/commands/select`] Improved `select` arguments with addition new argument `--fuzzy_max_distance_ratio`.

  This argument computes --fuzzy_max_distance based on the input keyword dynamically by `floor(THE_NUMBER_OF_CHARACTERS * FUZZY_MAX_DISTANCE)`.
  We recommend taht the value of `FUZZY_MAX_DISTANCE` is 0.34.

* [{doc}`/reference/commands/select`] Improved `select` arguments with addition new argument `--fuzzy_prefix_length`.

  This argument set keyword length to use fuzzy prefix search.
...
* [fuzzy_search] Fix a bug that Groonga may get records that should not match.
