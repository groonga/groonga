...
* [{doc}`/reference/commands/column_create`] Improved `column_create` flags with addition new flags `COLUMN_FILTER_SHUFFLE` and `COLUMN_FILTER_BYTE_DELTA`.

* [{doc}`/reference/commands/column_create`] Improved `column_create` flags with addition new flags `COMPRESS_FILTER_TRUNCATE_PRECISION_1BYTE` and `COMPRESS_FILTER_TRUNCATE_PRECISION_2BYTES`.
