...
* [{doc}`/reference/commands/column_create`] Added newly flags `COLUMN_FILTER_SHUFFLE` and `COLUMN_FILTER_BYTE_DELTA`.

* [{doc}`/reference/commands/column_create`] Added newly flags `COMPRESS_FILTER_TRUNCATE_PRECISION_1BYTE` and `COMPRESS_FILTER_TRUNCATE_PRECISION_2BYTES`.

* Added newly bundling library Blosc.

  `COLUMN_FILTER_SHUFFLE` and `COLUMN_FILTER_BYTE_DELTA` flags are require Blosc.

* Rename master branch to main branch.

* [RPM] Added support for building with CMake.

* [CI] Added support for Debian trixie on CI.

* [{doc}`/reference/commands/status`] Added a newly features "blosc".

* [{doc}`/reference/executables/groonga`] Added "blosc" in "--version".

* [{doc}`/reference/commands/select`] Added a newly argument "--fuzzy_max_distance".

* [{doc}`/reference/commands/select`] Added a newly argument "--fuzzy_max_expansion".

* [cast] Added support for casting [...] to Float/Float32 vector.
