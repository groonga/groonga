...
* [:doc:`/reference/commands/select`] Added support for large drilldown keys.
  ...
  * The maximum on the key size of Groonga's tables are 4KiB.
    ...
    * Because the drilldown packets specifying all keys for drilldown. So, if each the size of drilldown key is large,
...
* Fixed the following bugs related multi column index.
  ...
  * Please refer to the following URL for the details about occurrence conditions of this bug.
