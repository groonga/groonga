...
* [{doc}`/reference/commands/load`] Stopped reporting an error for key that becomes an empty key by normalization if we load data that empty string after normalize.
  ...
  Because Groonga output many "empty key" error because of Groonga can't regist empty string to index.

  No problem even if empty string can't register to index in such case.
  Because we don't match anything even if we search by empty string.
