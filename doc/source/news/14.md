...
意味ベースの検索ができるようになると、同義語展開用のテーブルを準備、メンテナンスせずに意味の近い言葉を検索できるようになります。

###### [column_create] 新しい引数 `--generator` を追加しました。

カラムを自動生成できます。
他のカラムの値からカラムの値を自動で生成できます。
この機能のメリットは以下の2点です。

1. わざわざユーザーがテキスト生成処理を管理する必要がありません。
2. 生成元のカラムさえあれば良いので、`load`するデータ量を減らすことができます。

例えば、他のカラムの値を`language_model_vectorize()`でエンべディングして、その値を自動生成したカラムに格納するということもできます。
これを利用すれば、ユーザーは検索対象のテキストを`load`するだけで、エンべディングされたデータも同時に格納できるようになります。
