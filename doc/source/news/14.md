...
###### [status] Added `llama.cpp` to the `status` results.

The status results display whether `llama.cpp` is enabled or disabled under the `features` section, as follows.
(Only the relevant parts are extracted for explanation due to the lengthy status output.)
...
The `llama.cpp` feature is used in the newly introduced `language_model_vectorize()` function described next.

###### [`language_model_vectorize()`] Added new function `language_model_vectorize()` (Experimental)

This feature is still experimental and unstable.
It also consumes a significant amount of CPU and memory.

The function returns normalized embeddings (vector representations) of the given text.
An embedding represents the semantic information of the text as a vector.
By converting meanings of the text into vectors, you can perform semantic searches instead of literal text searches.

For example, "牛乳" ("milk" in kanji) and "ミルク" ("milk" in katakana) look completely different but share the same meaning.
Traditionally, you could search for words with different spellings but the same meaning by using synonym expansion.
But this required maintaining a synonym table.

With semantic search enabled, you can find words with similar meanings without preparing or maintaining a synonym table.

###### [{doc}`/reference/commands/column_create`] Added a new `--generator` argument.

You can now automatically generate columns based on the values of other columns.
The benefits of this feature include:

1. Users no longer need to manage text generation processes manually.
2. You can reduce the amount of data to load since only the source columns are necessary.

For example, you can use `language_model_vectorize()` to embed the values of other columns and store them in automatically generated columns.
This allows users to `load` only the text to be searched while simultaneously storing the embedded data.

### Fixes

###### [{doc}`/server/gqtp`] Fixed an issue where the `RC` value of `INVALID_ARGUMENT` returned to the GQTP client was `65514` instead of `-22`.

This fix does not affect users who are not using GQTP clients.

###### [{doc}`/reference/commands/logical_range_filter`] Fixed an issue where an extra `"records": []` was output in the response when using command version 3.

This issue happens only with command version 3. The default command version is 1.
You can check your Groonga's command version using the `status` command.
...
As a follow, an extra `"records": []` was output.
This does not happen if you are not using command version 3.
...
```
{
  ...
  "header": {
    "return_code": 0,
    "start_time": 1729647532.743954,
    "elapsed_time": 0.001991033554077148
  }
}
```

###### [{doc}`/reference/commands/logical_range_filter`] Fixed an issue where the response became invalid JSON when a request was canceled using {doc}`/reference/commands/request_cancel`.

A bug was fixed where the response output could become invalid JSON depending on the timing of the cancellation.

###### [{doc}`/reference/commands/index_column_diff`] Fixed an issue where index corruption was falsely detected when a single record contained a large number of identical tokens.

This issue occurs when a single record contains more than 131,071 instances of the same token.
There's no easy way to check the number of tokens.
If you regularly use `index_column_diff` to detect index corruption, we recommend updating as a precaution.
