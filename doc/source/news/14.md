...
#### {doc}`/reference/commands/delete`: Added support for referenced weight vector

If a referenced column is indexed, `delete` can also delete the
deleted referenced value from the referenced column. It works for a
referenced vector column but didn't work for a referenced "weight"
vector column.

With this release, this also works for a referenced "weight" vector
column.
