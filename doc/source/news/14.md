...
* [{doc}`/reference/functions/query_parallel_or`] Fixed a bug where the [{ref}`select-match-escalation-threshold`] or `force_match_escalation` options were ignored when using `query_parallel_or`.
  ...
  This problem occured only `query_parallel_or()`. `query()` didn't occur this problem.
...
* Fixed a bug where full-text search on a reference column of a vector didn't work.
  ...
  For exampe, before the fix, Groonga always returned 0 hits when performing full-text search on a reference column of a vector. Internally, this happened because Groonga couldn't correctly retrieve `key_type` for reference columns.
