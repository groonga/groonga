...
(release-14-0-7)=
## Release 14.0.7 - 2024-08-29

### Improvements

* [NormalizerNFKC] Added a new option `unify_alphabet_diacritical_mark`.[pgroonga/pgroonga#418][Reported by Man Hoang]
  ...
  table_create \
    ...
    --normalizer NormalizerNFKC150("unify_alphabet_diacritical_mark", true)
  column_create \
    --table Terms \
    --name index_content \
...
* Added support for simdjson.
  ...
  This keeps RapidJSON support but we'll remove RapidJSON support later.
...
* Fixed a crash bug when log rotate is enabled.
  ...
  This bus occurred in the following conditions.
  ...
  2. Log rotate is enabled by specification the following options.

    * `--log-rotate-threshold-size <threshold>`
    * `--query-log-rotate-threshold-size <threshold>`

  3. Process ID log output is enabled by specification the following options.

    * `--log-flags process_id`

* Fixed a bug if we specify `GRN_WITH_BUNDLED_ONIGMO=OFF`, Groonga use bundled Onigmo. [termux/termux-packages#21252][Patch by Biswapriyo Nath]
