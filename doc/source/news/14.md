...
検索対象のドキュメントをベクトルに変換できます。
検索対象のドキュメントをベクトルとして保持できると、字面ではなく意味ベースの検索ができるようになります。

例えば、「牛乳」と「ミルク」は字面は全然違いますが、同じ意味を持つ言葉です。
従来でも同義語展開を使えば、「牛乳」と「ミルク」のような字面は異なるが意味が同じ言葉を検索できますが、同義語展開用のテーブルをメンテナンスしなければなりません。

意味ベースの検索ができるようになると、同義語展開用のテーブルを準備、メンテナンスなどの特別なことをしなくても、Groongaがいい感じに近い言葉を探してくれるようになります。

###### [column_create] 新しい引数 `--generator` を追加しました。

カラムを自動生成できます。
以下のように他のカラムの値からカラムの値を自動で生成できます。
この機能のメリットは以下の2点です。
