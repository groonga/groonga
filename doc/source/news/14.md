...
* Fixed a bug that rehash failed or Groonga's data broke when records existed 2^28 or more in table with `TABLE_HASH_KEY`.

* Fixed a bug that highlight position slipped out of place in the following cases.

  * If highlight target charcters include multibyte character.(要確認)

  * If white space existed before highlight target charcters.

  * If we used `TokenNgram("loose_blank", true)` and if highlight target charcters included full width space.

  * If white space existed in the front of highlight target charcters.

  * If white space existed in front and behind of highlight target charcters like ` a `.

  * If the second charcter of highlight target was full width space.

  * If the order of highlight target charcters was such as `${SPACE}${CHARACTER}${FULL_WIDTH_SPACE}${CHARACTER}`.

* Fix a bug that alphabet before white space may not highlight in the following cases.
