...
* [{doc}`/reference/commands/request_cancel`] Fix a bug that Groonga may crash when we execute `request_cancel` command whlie we execute the other query.
...
* Fixed a bug that rehash failed or data in a table broke when rehash occured that the table with `TABLE_HASH_KEY` has  2^28 or more records.

* Fixed a bug that highlight position slipped out of place in the following cases.

  * If full width space existed before highlight target charcters as below.
  ...
  * If we used `TokenNgram("loose_blank", true)` and if highlight target charcters included full width space as below.
  ...
  * If white space existed in the front of highlight target charcters as below.
  ...
  * If the second charcter of highlight target was full width space as below.
