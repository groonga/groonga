...
* [{doc}`/reference/functions/query_parallel_or`] Fixed a bug where the {ref}`select-match-escalation-threshold` or `force_match_escalation` options were ignored when using `query_parallel_or`.

  Before the fix, even when `match_escalation_threshold` was set to disable match escalation, the results still escalated.
  This problem occured only `query_parallel_or()`. `query()` don't occur this problem.
