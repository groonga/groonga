 ...
 -- #{name} <#{email}>  #{date}
  ...
  def define_release_tasks
    ...
    namespace :release do
      ...
      def org_path
        File.expand_path(org_dir)
      end

      def doc_path
        "#{dir}/**/share/doc/groonga/*/"
      end
        ...
        task :update do
          ...
          Dir.mktmpdir do |dir|
            sh({"DESTDIR" => dir},
               ...
               "--build", build_dir,
               "--target", "install")
            Dir.glob(doc_path) do |doc|
              locale = File.basename(doc)
              if locale == "en"
                org_docs = File.join(org_path, "docs")
              else
                org_docs = File.join(org_path, locale, "docs")
              end
              rm_rf(org_docs)
              mv(doc, org_docs)
              rm_f(File.join(org_docs, ".buildinfo"))
            end
          end
        end
      end
