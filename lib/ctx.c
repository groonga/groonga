...
grn_ctx_fin(grn_ctx *ctx)
{
  ...
  if (ctx->impl) {
    ...
    if (ctx->impl->com) {
      ...
      rc = grn_com_sqtp_close(ctx, NULL, ctx->impl->com);
    }
...
grn_ql_send(grn_ctx *ctx, char *str, unsigned int str_len, int flags)
{
  ...
  if (ctx->impl) {
    if (ctx->impl->com) {
      ...
      if ((rc = grn_com_sqtp_send(ctx, ctx->impl->com, &sheader, (char *)str))) {
        ERR(rc, "grn_com_sqtp_send failed");
      }
      goto exit;
    } else {
      if (ctx->impl->symbols) {
    ...
    if (ctx->impl->com) {
      if (grn_com_sqtp_recv(ctx, ctx->impl->com, &ctx->impl->com->msg, &ctx->impl->com_status, &ctx->impl->com_info)) {
        ...
        *flags = 0;
        ...
        grn_com_sqtp_header *rheader = GRN_COM_SQTP_MSG_HEADER(&ctx->impl->com->msg);
        *str = GRN_COM_SQTP_MSG_BODY(&ctx->impl->com->msg);
      ...
      if (ctx->impl->com->rc) {
        ERR(ctx->impl->com->rc, "grn_com_sqtp_recv failed!");
      }
      goto exit;
    } else {
      if (ctx->impl->symbols) {
