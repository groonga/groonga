...
async function createFunctionPointer(exports, callbackModule, name, callback) {
...
export class Groonga {
  // Initialize libgroonga.wasm. `wasi` must have getImportObject()
  // and initialize() like WASI in node:wasi. `module` and
  // `callbackModule` are compiled libgroonga.wasm and
  // libgroonga-callback.wasm. Use open() in libgroonga-node.mjs or
  // libgroonga-web.mjs to prepare them.
  //
  // `logPath` and `queryLogPath` must be set before grn_init(). So
  // they are options of this instead of setters.
  static async open({
    wasi,
    module,
    callbackModule,
