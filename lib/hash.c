               ...
               uint32_t value_size, uint32_t flags)
{
  ...
  GRN_OBJ_DB_SET_TYPE(array, GRN_TABLE_NO_KEY);
...
inline static grn_rc
io_array_init(grn_array *array, grn_ctx *ctx, const char *path,
               uint32_t value_size, uint32_t flags)
{
  grn_io *io;
  ...
  for (w_of_element = 0; (1 << w_of_element) < value_size; w_of_element++);
  {
    ...
    io = grn_io_create_with_array(path, sizeof(struct grn_array_header),
...
grn_array_create(grn_ctx *ctx, const char *path, uint32_t value_size, uint32_t flags)
{
  ...
  if (ctx && (array = GRN_MALLOC(sizeof(grn_array)))) {
    if (!((flags & GRN_ARRAY_TINY) ?
          ...
          io_array_init(array, ctx, path, value_size, flags))) {
      return array;
    }
    GRN_FREE(array);
  }
...
{
  if (ctx) {
    grn_io *io = grn_io_open(path, grn_io_auto);
    if (io) {
...
grn_array_cursor_open(grn_ctx *ctx, grn_array *array, grn_id min, grn_id max, int flags)
{
  ...
  GRN_OBJ_DB_SET_TYPE(c, GRN_CURSOR_TABLE_NO_KEY);
  c->array = array;
  c->ctx = ctx;
  c->obj.flags = flags;
...
array_entry_new(grn_array *array)
{
  grn_id e;
  if (IO_ARRAYP(array)) {
    struct grn_array_header *hh = array->header;
...
{
  grn_id e;
  if (!ctx || !array || !(e = array_entry_new(array))) { return GRN_ID_NIL; }
  (*array->n_entries)++;
  if (value) {
    void *ee;
...
inline static grn_id *
idx_at_(grn_hash *hash, grn_id id)
{
...
#define IDX_AT(h,i) (\
  IO_HASHP(h) ?\
  idx_at_(h, ((i) & *(h)->max_offset) + h->header->idx_offset) :\
...
inline static char *
get_key(grn_hash *hash, entry_str *n)
{
...
put_key_(grn_hash *hash, const char *key, int len)
{
  uint32_t res, ts;
...
inline static grn_rc
put_key(grn_hash *hash, entry_str *n, uint32_t h, const char *key, unsigned int len)
{
  n->key = h;
  if (hash->obj.flags & GRN_OBJ_KEY_VAR_SIZE) {
    if (IO_HASHP(hash)) {
...
inline static int
match_key(grn_hash *hash, entry_str *ee, uint32_t h, const char *key, unsigned int len)
{
  if (hash->obj.flags & GRN_OBJ_KEY_VAR_SIZE) {
    return (ee->key == h &&
            ee->size == len &&
            !memcmp(key, get_key(hash, ee), len));
  } else {
    return (ee->key == h &&
            ((len == sizeof(uint32_t)) ||
             !memcmp(key, get_key(hash, ee), len)));
               ...
               uint32_t init_size)
{
  ...
  for (w_of_element = 0; (1 << w_of_element) < entry_size; w_of_element++);
  {
    ...
    io = grn_io_create_with_array(path, GRN_HASH_HEADER_SIZE, GRN_HASH_SEGMENT_SIZE,
  ...
  GRN_OBJ_DB_SET_TYPE(ih, GRN_TABLE_HASH_KEY);
  ih->obj.flags = flags;
  ih->ctx = ctx;
               ...
               uint32_t value_size, uint32_t flags, grn_encoding encoding)
{
  ...
  GRN_OBJ_DB_SET_TYPE(ah, GRN_TABLE_HASH_KEY);
  ah->obj.flags = flags;
  ah->ctx = ctx;
    ...
    if (!memcmp(header->idstr, GRN_IDSTR, 16)) {
      ...
      if (hash) {
        if (!(header->flags & GRN_HASH_TINY)) {
          GRN_OBJ_DB_SET_TYPE(hash, GRN_TABLE_HASH_KEY);
          hash->obj.flags = header->flags;
          hash->ctx = ctx;
...
grn_hash_reset(grn_ctx *ctx, grn_hash *hash, uint32_t ne)
{
  ...
  if (IO_HASHP(hash)) {
    ...
    for (i = 0; i < n; i += (IDX_MASK_IN_A_SEGMENT + 1)) {
      dp = idx_at_(hash, i + offd); // todo : use idx_at
...
inline static grn_id
entry_new(grn_hash *hash, uint32_t size)
{
  grn_id e;
  if (IO_HASHP(hash)) {
             ...
             grn_search_flags *flags)
{
  ...
  for (i = h; ; i += s) {
    ...
    ENTRY_AT(hash, e, ee, GRN_TABLE_ADD);
    if (!ee) { return GRN_ID_NIL; }
    if (match_key(hash, ee, h, key, key_size)) { goto exit; }
  }
  if (!(e = entry_new(hash, key_size))) { /* unlock */ return GRN_ID_NIL; }
  ENTRY_AT(hash, e, ee, GRN_TABLE_ADD);
  if (!ee) { return GRN_ID_NIL; }
  put_key(hash, ee, h, key, key_size);
  if (np) {
    (*hash->n_garbages)--;
    ep = np;
  }
  *ep = e;
  (*hash->n_entries)++;
  if (flags) { *flags |= GRN_TABLE_ADDED; }
exit :
  /* unlock */
...
grn_hash_at(grn_ctx *ctx, grn_hash *hash, const void *key, int key_size, void **value)
{
  ...
  for (i = h; ; i += s) {
    ...
    if (e == GARBAGE) { continue; }
    {
      ...
      if (ee && match_key(hash, ee, h, key, key_size)) {
        if (value) { *value = get_value(hash,ee); }
        return e;
      }
    }
...
const char *
_grn_hash_key(grn_hash *hash, grn_id id, uint32_t *key_size)
{
  uint8_t res;
  entry_str *ee;
...
grn_hash_get_key(grn_ctx *ctx, grn_hash *hash, grn_id id, void *keybuf, int bufsize)
{
  ...
  if (!ee) { return 0; }
  key_size = (hash->obj.flags & GRN_OBJ_KEY_VAR_SIZE) ? ee->size : hash->key_size;
  if (bufsize >= key_size) { memcpy(keybuf, get_key(hash, ee), key_size); }
  return key_size;
}
...
#define DELETE_IT {\
  *ep = GARBAGE;\
  if (IO_HASHP(hash)) {\
    ...
    hash->garbages = e;\
    if ((hash->obj.flags & GRN_OBJ_KEY_VAR_SIZE) && !(ee->flag & HASH_IMMEDIATE)) {\
      grn_ctx *ctx = hash->ctx;\
                    ...
                    const void *max, uint32_t max_size, int flags)
{
  ...
  GRN_OBJ_DB_SET_TYPE(c, GRN_CURSOR_TABLE_HASH_KEY);
  c->hash = hash;
  c->ctx = ctx;
  c->obj.flags = flags;
  if (flags & GRN_CURSOR_ASCENDING) {
...
#define PREPARE_VAL(e,ep,es) {\
  if ((arg->flags & GRN_TABLE_SORT_BY_VALUE)) {\
    ...
    ep = ((const uint8_t *)(get_key(hash, (entry_str *)(e))));\
    es = ((hash->obj.flags & GRN_OBJ_KEY_VAR_SIZE)\
...
inline static grn_id
entry2id(grn_hash *hash, entry *e)
{
  entry *e2;
  grn_id id, *ep;
              ...
              int limit, grn_array *result, grn_table_sort_optarg *optarg)
{
  ...
  if (optarg) {
    ...
    if ((optarg->flags & GRN_TABLE_SORT_BY_ID) ||
        ...
        : (!(hash->obj.flags & GRN_OBJ_KEY_VAR_SIZE)
...
grn_rhash_group(grn_hash *s, int limit, grn_group_optarg *optarg)
{
  ...
  while ((rh = grn_hash_cursor_next(ctx, c))) {
    ...
    }
    {
      ...
      if (grn_hash_get(ctx, g, gkey, rsize, (void **)&gri, &f)) {
                     ...
                     grn_id *rid, int *section, int *pos, int *score, void **subrec)
{
  ...
  if ((unsigned int)index >= s->max_n_subrecs) { return grn_invalid_argument; }
  {
    ...
    ENTRY_AT(s, rh, ee, 0);
    if (!ee) { return grn_invalid_argument; }
    pi = (grn_rset_posinfo *)get_key(s, ee);
    ri = get_value(s, ee);
