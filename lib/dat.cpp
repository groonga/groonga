...
namespace {
  ...
  public:
    ...
    void
    enter(grn_critical_section *lock)
    {
      leave();
      lock_ = lock;
    }
  ...
  private:
    ...
    CriticalSection &
    operator=(const CriticalSection &);
  };
  ...
  grn_dat_remove_file(grn_ctx *ctx, const char *path)
  {
    ...
    GRN_LOG(ctx, GRN_LOG_INFO, "[dat][remove-file] removed: <%s>", path);
    return true;
  }
   ...
   */
  void
  grn_dat_generate_trie_path(const char *base_path,
                             char *trie_path,
                             uint32_t file_id)
  {
  ...
  grn_dat_open_trie_if_needed(grn_ctx *ctx, grn_dat *dat)
  {
    ...
    grn::dat::Trie *const old_trie =
      static_cast<grn::dat::Trie *>(dat->old_trie);
  ...
  bool
  grn_dat_rebuild_trie(grn_ctx *ctx, grn_dat *dat)
  {
  ...
  grn_id
  grn_dat_add_internal(grn_ctx *ctx,
                       grn_dat *dat,
                       const void *key,
  ...
  grn_rc
  grn_dat_delete_by_id_internal(grn_ctx *ctx, grn_dat *dat, grn_id id)
  {
    try {
      grn::dat::Trie *const trie = static_cast<grn::dat::Trie *>(dat->trie);
      if (!trie->remove(id)) {
  ...
  grn_rc
  grn_dat_delete_internal(grn_ctx *ctx,
                          grn_dat *dat,
                          const void *key,
                          unsigned int key_size)
  {
  ...
  grn_rc
  grn_dat_update_by_id_internal(grn_ctx *ctx,
                                grn_dat *dat,
                                grn_id src_key_id,
                                const void *dest_key,
  ...
  grn_rc
  grn_dat_update_internal(grn_ctx *ctx,
                          grn_dat *dat,
                          const void *src_key,
  ...
  void
  grn_dat_cursor_init(grn_ctx *, grn_dat_cursor *cursor)
  {
  ...
  void
  grn_dat_cursor_fin(grn_ctx *, grn_dat_cursor *cursor)
  {
  ...
  void
  grn_dat_wal_set_wal_id(grn_ctx *, grn_dat *dat, uint64_t id)
  {
  ...
  private:
    ...
    WALRecorder &
    operator=(const WALRecorder &);

    grn_rc
    record_add_entry(Used &used)
    {
      used.key = true;
    ...
    std::string
    format_details(const Used &used)
    {
      std::ostringstream details;
      details << "event:" << event << "<" << grn_wal_event_to_string(event)
              << "> ";
      if (used.record_id) {
  ...
  public:
    ...
    recover()
    {
      ...
      auto reader =
        grn_wal_reader_open(ctx_, reinterpret_cast<grn_obj *>(dat_), tag_);
      if (ctx_->rc != GRN_SUCCESS) {
        return ctx_->rc;
      }
        ...
        while (true) {
          ...
          case GRN_WAL_EVENT_DELETE_ENTRY:
            ...
            if (entry.record_id == GRN_ID_NIL) {
              ...
              grn_dat_delete_by_id_internal(ctx_, dat_, entry.record_id);
            }
            break;
          case GRN_WAL_EVENT_UPDATE_ENTRY:
            if (entry.record_id == GRN_ID_NIL) {
...
grn_dat *
grn_dat_create(
  grn_ctx *ctx, const char *path, uint32_t, uint32_t, uint32_t flags)
{
  if (path) {
    if (path[0] == '\0') {
      path = NULL;
  ...
  dat->io = grn_io_create(ctx,
                          path,
                          sizeof(struct grn_dat_header),
                          4096,
                          0,
                          GRN_IO_AUTO,
  ...
  const grn_encoding encoding =
    (ctx->encoding != GRN_ENC_DEFAULT) ? ctx->encoding : grn_gctx.encoding;
...
grn_id
grn_dat_get(
  grn_ctx *ctx, grn_dat *dat, const void *key, unsigned int key_size, void **)
{
  if (!grn_dat_open_trie_if_needed(ctx, dat)) {
    return GRN_ID_NIL;
  }
  const grn::dat::Trie *const trie =
    static_cast<const grn::dat::Trie *>(dat->trie);
  if (!trie) {
    return GRN_ID_NIL;
  }
  grn::dat::UInt32 key_pos;
...
grn_rc
grn_dat_delete_by_id(grn_ctx *ctx,
                     grn_dat *dat,
                     grn_id id,
               ...
               grn_table_delete_optarg *optarg)
{
  ...
  if (optarg && optarg->func) {
    try {
      const grn::dat::Trie *const trie =
        static_cast<grn::dat::Trie *>(dat->trie);
      grn::dat::UInt32 key_pos;
      if (!trie->search(key, key_size, &key_pos)) {
        return GRN_INVALID_ARGUMENT;
      } else if (!optarg->func(ctx,
...
grn_rc
grn_dat_update_by_id(grn_ctx *ctx,
                     grn_dat *dat,
                     grn_id src_key_id,
                     const void *dest_key,
                     unsigned int dest_key_size)
...
grn_id
grn_dat_lcp_search(grn_ctx *ctx,
                   grn_dat *dat,
                   const void *key,
                   unsigned int key_size)
{
...
grn_dat_cursor *
grn_dat_cursor_open(grn_ctx *ctx,
                    grn_dat *dat,
                    const void *min,
                    ...
                    int flags)
{
  ...
  try {
    if ((flags & GRN_CURSOR_BY_ID) != 0) {
      dc->cursor = grn::dat::CursorFactory::open(
        *trie,
        min,
        min_size,
        max,
          ...
          dc->cursor = grn::dat::CursorFactory::open(
            *trie,
            ...
            grn::dat::PREDICTIVE_CURSOR |
              ((flags & GRN_CURSOR_DESCENDING) ? grn::dat::DESCENDING_CURSOR
                                               : 0) |
...
grn_rc
grn_dat_cursor_delete(grn_ctx *ctx,
                      grn_dat_cursor *c,
                      grn_table_delete_optarg *optarg)
{
  if (!c || !c->cursor) {
...
grn_dat_is_corrupt(grn_ctx *ctx, grn_dat *dat)
{
...

  {
    ...
    if (::stat(trie_path, &stat) != 0) {
      SERR("[dat][corrupt] used path doesn't exist: <%s>", trie_path);
      return true;
    }
  }
