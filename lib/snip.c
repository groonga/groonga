                ...
                grn_encoding enc, int flags)
{
  ...
  if (!(sc->keyword = grn_str_open(ctx, keyword, keyword_len, enc, f))) {
    GRN_LOG(grn_log_alert, "grn_str_open on snip_cond_init failed !");
              ...
              grn_snip_mapping *mapping)
{
  ...
  if (!(ret = GRN_MALLOC(sizeof(grn_snip)))) {
    GRN_LOG(grn_log_alert, "grn_snip allocation failed on grn_snip_open");
    return NULL;
  }
  if (max_results > MAX_SNIP_RESULT_COUNT || max_results == 0) {
    GRN_LOG(grn_log_warning, "max_results is invalid on grn_snip_open");
    GRN_FREE(ret);
    return NULL;
  }
              ...
              unsigned int *nresults, unsigned int *max_tagged_len)
{
  ...
  *nresults = 0;
  ...
  if (!snip->nstr) {
    exec_clean(ctx, snip);
    GRN_LOG(grn_log_alert, "grn_str_open on grn_snip_exec failed !");
    return GRN_NO_MEMORY_AVAILABLE;
  }
