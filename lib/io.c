...
grn_io_init(void)
{
  ...
  if (!grn_dls) { return grn_memory_exhausted; }
  return grn_success;
}
    ...
    case grn_io_wronly:
      {
        ...
        for (p = iw->addr, r = iw->size; r; p += s, r -= s, segment++, offset = 0) {
          GRN_IO_SEG_MAP(io, segment, q);
          if (!q) { return grn_memory_exhausted; }
          s = (offset + r > segment_size) ? segment_size - offset : r;
          memcpy(q + offset, p, s);
        }
      }
...
grn_open(grn_ctx *ctx, fileinfo *fi, const char *path, int flags, size_t maxsize)
{
  ...
  if (fi->fh == INVALID_HANDLE_VALUE) {
    GERR(grn_file_operation_error, "CreateFile failed");
    return grn_file_operation_error;
  }
  if ((flags & O_TRUNC)) {
    CloseHandle(fi->fh);
...
grn_open(grn_ctx *ctx, fileinfo *fi, const char *path, int flags, size_t maxsize)
{
  ...
  CloseHandle(fi->fh);
  GERR(grn_file_operation_error, "OpenFileMapping failed");
  return grn_file_operation_error;
}
...
grn_pread(grn_ctx *ctx, fileinfo *fi, void *buf, size_t count, off_t offset)
{
  ...
  if (r != count) {
    if (r == -1) {
      ...
      GERR(grn_file_operation_error, "pread returned %d != %d", r, count);
    }
    return grn_file_operation_error;
  }
  return grn_success;
...
grn_pwrite(grn_ctx *ctx, fileinfo *fi, void *buf, size_t count, off_t offset)
{
  ...
  if (r != count) {
    if (r == -1) {
      ...
      GERR(grn_file_operation_error, "pwrite returned %d != %d", r, count);
    }
    return grn_file_operation_error;
  }
  return grn_success;
