...
#ifndef WIN32
#  ifdef HAVE_FUTIMENS
#    define grn_futimens(fd, time_values, time_specs)                          \
      futimens((fd), (time_specs))
#    define grn_futimens_name "futimens"
#  elif defined(HAVE_FUTIMES) /* HAVE_FUTIMENS */
#    define grn_futimens(fd, time_values, time_specs)                          \
      futimes((fd), (time_values))
...
static inline int
grn_munmap(
  grn_ctx *ctx, grn_io *io, fileinfo *fi, grn_io_mapinfo *mi, size_t length);
#define GRN_MMAP(ctx, io, fi, mi, offset, length)                              \
  (grn_mmap((ctx),                                                             \
            (io),                                                              \
            (fi),                                                              \
            (mi),                                                              \
            ...
            __LINE__,                                                          \
            __FUNCTION__))
#define GRN_MUNMAP(ctx, io, fi, mi, length)                                    \
  (grn_munmap((ctx), (io), (fi), (mi), (length)))
...
{
  {
    ...
    grn_getenv("GRN_IO_VERSION", version_env, GRN_ENV_BUFFER_SIZE);
    if (version_env[0]) {
...
static uint32_t
grn_io_compute_max_n_files(uint32_t segment_size,
                           uint32_t max_segment,
                           unsigned int base_segument,
                           uint32_t file_size)
{
                  ...
                  uint32_t flags)
{
  ...
  grn_io_mapinfo mi;
  grn_io_mapinfo_init(&mi);
  b = grn_io_compute_base(header_size);
  header = (struct _grn_io_header *)GRN_MMAP(ctx, NULL, NULL, &mi, 0, b);
  if (header) {
...
grn_io_register(grn_ctx *ctx, grn_io *io)
{
  if (io->fis && (io->flags & (GRN_IO_EXPIRE_GTICK | GRN_IO_EXPIRE_SEGMENT))) {
    ...
    if (!succeeded) {
      GRN_LOG(ctx, GRN_LOG_WARNING, "grn_io_register(%s) failed", io->path);
    }
  }
}
...
grn_io_unregister(grn_ctx *ctx, grn_io *io)
{
  if (io->fis && (io->flags & (GRN_IO_EXPIRE_GTICK | GRN_IO_EXPIRE_SEGMENT))) {
    ...
    CRITICAL_SECTION_LEAVE(grn_glock);
    if (!succeeded) {
      GRN_LOG(ctx, GRN_LOG_WARNING, "grn_io_unregister(%s) failed", io->path);
    }
  }
}

grn_io *
grn_io_create(grn_ctx *ctx,
              const char *path,
              uint32_t header_size,
              uint32_t segment_size,
              uint32_t max_segment,
              ...
              uint32_t flags)
{
  ...
  if (path_length == 0 || (path_length > PATH_MAX - 4)) {
    return NULL;
  }
  b = grn_io_compute_base(header_size);
  ...
  if ((fis = GRN_MALLOCN(fileinfo, max_nfiles))) {
    ...
    if (!grn_fileinfo_open(ctx, fis, path, O_RDWR | O_CREAT | O_EXCL)) {
      grn_io_mapinfo mi;
      grn_io_mapinfo_init(&mi);
      header = (struct _grn_io_header *)GRN_MMAP(ctx, NULL, fis, &mi, 0, b);
#ifdef _WIN32
      fis->fmo = mi.fmo;
#endif
      if (header) {
      ...
      if (grn_unlink(path) == 0) {
        GRN_LOG(ctx,
                GRN_LOG_INFO,
                "[io][create][error] removed path: <%s>",
                path);
      } else {
...
static grn_rc
array_init_(
  grn_ctx *ctx, grn_io *io, uint32_t n_arrays, size_t hsize, size_t msize)
{
  uint32_t i;
  uint32_t ws;
  byte *hp, *mp;
...
grn_io *
grn_io_create_with_array(grn_ctx *ctx,
                         const char *path,
                         uint32_t header_size,
                         uint32_t segment_size,
                         grn_io_mode mode,
                         ...
                         grn_io_array_spec *array_specs)
{
  if (n_arrays) {
    ...
    if ((io = grn_io_create(ctx,
                            path,
                            (uint32_t)(header_size + hsize),
                            segment_size,
                            nsegs,
                            mode,
                            GRN_IO_EXPIRE_GTICK))) {
...
void
grn_io_segment_alloc(grn_ctx *ctx,
                     grn_io *io,
                     grn_io_array_info *ai,
                     uint32_t lseg,
                     int *flags,
...
grn_io_detect_type(grn_ctx *ctx, const char *path)
{
  ...
  if (fd != -1) {
      ...
      if (grn_read(fd, &h, sizeof(struct _grn_io_header)) ==
        ...
        if (!memcmp(h.idstr, GRN_IO_IDSTR, GRN_IO_IDSTR_LEN)) {
          ...
          ERR(GRN_INCOMPATIBLE_FILE_FORMAT,
              ...
              path,
              (int)GRN_IO_IDSTR_LEN,
              GRN_IO_IDSTR);
        }
      } else {
        SERR("failed to read enough data for detecting type: <%s>", path);
      }
    } else {
    ...
    ERRNO_ERR("failed to open path for detecting type: <%s>", path);
  }
  return res;
}
...
grn_io_open(grn_ctx *ctx, const char *path, grn_io_mode mode)
{
  ...
  }
  {
    ...
    if (fd == -1) {
      ERRNO_ERR("[io][open] failed to open path: <%s>", path);
      return NULL;
    }
    if (fstat(fd, &s) == -1) {
      ERRNO_ERR("[io][open] failed to file status: <%s>", path);
      grn_close(fd);
      return NULL;
    }
  ...
  if (rc != GRN_SUCCESS) {
    ERR(rc, "[io][open] failed to open fileinfo: <%s>", path);
    return NULL;
  }
  ...
  grn_io_mapinfo mi;
  grn_io_mapinfo_init(&mi);
  header = GRN_MMAP(ctx, NULL, &fi, &mi, 0, b);
#ifdef _WIN32
  fi.fmo = mi.fmo;
#endif
  if (!header) {
  ...
  if (io->maps) {
    ...
    for (i = 0; i < max_segment; i++) {
      ...
      if (mi->map) {
        ...
        GRN_MUNMAP(ctx, io, fi, mi, segment_size);
      }
    }
    GRN_FREE(io->maps);
  }
...
#endif
    GRN_MUNMAP(ctx, io, io->fis, &mi, io->base);
#ifdef _WIN32
    if (io->fis) {
      io->fis->fmo = NULL;
    }
...
grn_io_size(grn_ctx *ctx, grn_io *io, uint64_t *size)
{
  ...
  for (fno = 0; fno < n_files; fno++) {
    ...
    if (stat(buffer, &s)) {
      SERR("failed to stat path to compute size: <%s>", buffer);
    } else {
...
grn_io_remove_raw(grn_ctx *ctx, const char *path)
{
  ...
  if (grn_unlink(path) != 0) {
    ERRNO_ERR("[io][remove] failed to remove path: <%s>", path);
    return ctx->rc;
  }
  ...
  for (fno = 1;; fno++) {
    ...
    if (grn_unlink(buffer) == 0) {
      GRN_LOG(ctx,
              GRN_LOG_INFO,
              "[io][remove] removed numbered path: <%d>: <%s>",
              fno,
              buffer);
    } else {
...
grn_io_rename(grn_ctx *ctx, const char *old_name, const char *new_name)
{
  ...
  struct stat s;
  if (stat(old_name, &s)) {
    SERR("%s failed to stat path to be renamed: <%s>", tag, old_name);
    return ctx->rc;
  } else if (rename(old_name, new_name)) {
    SERR("%s failed to rename path: <%s> -> <%s>", tag, old_name, new_name);
    return ctx->rc;
  } else {
    uint32_t fno;
                ...
                uint32_t value_len)
{
  ...
  if (rest) {
    ...
    do {
      ...
      size = rest > file_size ? file_size : rest;
      if ((rc = grn_pwrite(ctx, fi, vr, size, 0))) {
        return rc;
      }
      vr += size;
      rest -= size;
...
grn_rc
grn_io_write_ja_ehead(grn_io *io,
                      grn_ctx *ctx,
                      uint32_t key,
                      uint32_t segment,
                      uint32_t offset,
...
grn_io_win_unmap(grn_ctx *ctx, grn_io_win *iw)
{
  ...
  if (iw->cached) {
    if (!iw->tiny_p) {
      grn_io_seg_unref(ctx, iw->io, iw->segment);
    }
    return GRN_SUCCESS;
  }
  {
...
#define DO_MAP(io, fi, mi, pos, size, segno)                                   \
  do {                                                                         \
    void *map = GRN_MMAP(ctx, (io), (fi), (mi), (pos), (size));                \
    if (map) {                                                                 \
      uint32_t nmaps;                                                          \
      if (io->max_map_seg < segno) {                                           \
        io->max_map_seg = segno;                                               \
      }                                                                        \
      GRN_ATOMIC_ADD_EX(&io->nmaps, 1, nmaps);                                 \
      {                                                                        \
        uint64_t tail = io->base + (uint64_t)(size) * ((segno) + 1);           \
        if (tail > io->header->curr_size) {                                    \
          io->header->curr_size = tail;                                        \
        }                                                                      \
      }                                                                        \
    }                                                                          \
  } while (0)
...
grn_io_seg_map_(grn_ctx *ctx, grn_io *io, uint32_t segno, grn_io_mapinfo *info)
{
  ...
  if ((io->flags & GRN_IO_TEMPORARY)) {
    DO_MAP(io, NULL, info, 0, segment_size, segno);
  } else {
    uint32_t file_size = grn_io_compute_file_size(io->header->version);
    uint32_t n_segments_per_file = file_size / segment_size;
    ...
    if (!grn_fileinfo_opened(fi)) {
      char path[PATH_MAX];
      gen_pathname(io->path, path, fno);
      bool path_exist = grn_path_exist(path);
      if (!grn_fileinfo_open(ctx, fi, path, O_RDWR | O_CREAT)) {
        DO_MAP(io, fi, info, pos, segment_size, segno);
        if (!info->map && !path_exist) {
          ...
          if (grn_unlink(path) == 0) {
            GRN_LOG(ctx,
                    ...
                    "removed created map file: <%s>",
                    path);
          } else {
            ERRNO_ERR("[io][map][error] memory mapping is failed and then "
                      "failed to remove created map file: <%s>",
                      path);
          }
        }
      }
...
grn_io_seg_expire(grn_ctx *ctx, grn_io *io, uint32_t segno, uint32_t nretry)
{
  ...
  grn_io_mapinfo *info;
  if (!io->maps || segno >= io->header->max_segment) {
    return GRN_INVALID_ARGUMENT;
  }
  info = &io->maps[segno];
  if (!info->map) {
  ...
  for (retry = 0;; retry++) {
    ...
    if (nref) {
      ...
      if (nref > 1) {
        ...
        GRN_MUNMAP(ctx, io, fi, info, io->header->segment_size);
        info->map = NULL;
        GRN_ATOMIC_ADD_EX(pnref, reset_max_ref_by_overflow, nref);
        GRN_ATOMIC_ADD_EX(&io->nmaps, -1, nmaps);
        GRN_FUTEX_WAKE(pnref);
...
grn_io_expire(grn_ctx *ctx, grn_io *io, uint32_t count_thresh, uint32_t limit)
{
  ...
  case GRN_IO_EXPIRE_SEGMENT:
    for (m = io->max_map_seg; n < limit && m; m--) {
      if (!grn_io_seg_expire(ctx, io, m, 0)) {
        n++;
      }
    }
    break;
      ...
      for (m = io->max_map_seg; n < limit && m; info++, m--) {
        if (info->map && (grn_gtick - info->count) > count_thresh) {
          ...
          if (!nref && info->map && (grn_gtick - info->count) > count_thresh) {
            GRN_MUNMAP(ctx, io, NULL, info, io->header->segment_size);
            GRN_ATOMIC_ADD_EX(&io->nmaps, -1, nmaps);
            info->map = NULL;
            info->count = grn_gtick;
            n++;
...
{
  return GRN_MMAP(ctx, NULL, NULL, mi, 0, length);
}

void
grn_io_anon_unmap(grn_ctx *ctx, grn_io_mapinfo *mi, size_t length)
{
  GRN_MUNMAP(ctx, NULL, NULL, mi, length);
}
...
grn_io_flush(grn_ctx *ctx, grn_io *io)
{
  ...
  if (io->maps) {
    ...
    for (i = 0; i < max_mapped_segment; i++) {
      ...
      msync_result =
        GRN_MSYNC(ctx, &(io->fis[nth_file_info]), info->map, segment_size);
      GRN_ATOMIC_ADD_EX(pnref, -1, nref);
      GRN_FUTEX_WAKE(pnref);
...
grn_io_is_corrupt(grn_ctx *ctx, grn_io *io)
{
  ...
  for (i = 0; i < n_files; i++) {
    ...
    if (stat(path, &s) != 0) {
      SERR("[io][corrupt] used path doesn't exist: <%s>", path);
      return true;
    }
  }
...
static inline void *
grn_mmap_v1(
  grn_ctx *ctx, fileinfo *fi, grn_io_mapinfo *mi, int64_t offset, size_t length)
{
  if (!fi) {
    mi->fmo = NULL;
  ...
  mi->map =
    MapViewOfFile(mi->fmo, FILE_MAP_WRITE, 0, (DWORD)offset, (SIZE_T)length);
  if (!mi->map) {
...
static inline int
grn_munmap_v1(grn_ctx *ctx, fileinfo *fi, grn_io_mapinfo *mi, size_t length)
{
  int r = 0;
  ...
  if (!FlushViewOfFile(mi->map, length)) {
    SERR("FlushViewOfFile(<%p>, <%" GRN_FMT_SIZE ">) failed on unmap",
         mi->map,
         length);
    r = -1;
  }
  if (UnmapViewOfFile(mi->map)) {
    mmap_size -= length;
  } else {
...
grn_fileinfo_open_v0(grn_ctx *ctx, fileinfo *fi, const char *path, int flags)
{
  ...
  if (fi->fmo == NULL) {
    ...
    if (fi->fmo == NULL) {
      /* try to create fmo */
      fi->fmo = CreateFileMapping(fi->fh,
                                  NULL,
                                  PAGE_READWRITE,
                                  0,
                                  GRN_IO_FILE_SIZE_V0,
  ...
  if (fi->fmo != NULL) {
    if (GetLastError() != ERROR_ALREADY_EXISTS) {
      ...
      GRN_LOG(ctx,
              GRN_LOG_ERROR,
              "fmo object already exists! handle=%p",
              fi->fh);
      CloseHandle(fi->fmo);
    }
  } else {
    GRN_LOG(ctx,
            GRN_LOG_ALERT,
            "failed to get FileMappingObject #%lu",
            GetLastError());
  }
  CloseHandle(fi->fh);
  ...
  grn_ctx *ctx, fileinfo *fi, grn_io_mapinfo *mi, int64_t offset, size_t length)
{
  ...
  /* file must be exceeded to GRN_IO_FILE_SIZE_V0 when FileMappingObject
  created. and, after fmo created, it's not allowed to expand the size of file.
  ...
  */
  ...
  mi->map =
    MapViewOfFile(fi->fmo, FILE_MAP_WRITE, 0, (DWORD)offset, (SIZE_T)length);
  if (!mi->map) {
...
static inline int
grn_munmap_v0(grn_ctx *ctx, fileinfo *fi, grn_io_mapinfo *mi, size_t length)
{
  int r = 0;
...
static inline bool
grn_fileinfo_open_common(grn_ctx *ctx,
                         fileinfo *fi,
                         const char *path,
                         int flags)
{
  ...
  if ((flags & O_CREAT)) {
    ...
    fi->fh = CreateFile(path,
                        GRN_IO_FILE_CREATE_MODE,
    ...
    if (fi->fh == INVALID_HANDLE_VALUE) {
      SERR("CreateFile(<%s>, <%s>) failed", path, flags_description);
      success = false;
      if (!exist) {
        struct stat stat_buffer;
        if (stat(path, &stat_buffer) == 0) {
    ...
    case CREATE_NEW:
      GRN_LOG(ctx, GRN_LOG_INFO, "[io][open] create new file: <%s>", path);
      break;
    case OPEN_ALWAYS:
      if (GetLastError() == ERROR_ALREADY_EXISTS) {
        GRN_LOG(ctx,
                GRN_LOG_INFO,
                "[io][open] open existing file because it exists: <%s>",
                path);
      } else {
        GRN_LOG(ctx,
                GRN_LOG_INFO,
  ...
  if ((flags & O_TRUNC)) {
    ...
    /* unable to assign OPEN_ALWAYS and TRUNCATE_EXISTING at once */
                        ...
                        NULL);
    if (fi->fh == INVALID_HANDLE_VALUE) {
      SERR("CreateFile(<%s>, <O_RDWR|O_TRUNC>) failed", path);
      success = false;
      goto exit;
    }
    GRN_LOG(ctx, GRN_LOG_INFO, "[io][open] truncated: <%s>", path);
    goto exit;
  }
  /* O_RDWR only */
  fi->fh = CreateFile(path,
                      ...
                      NULL);
  if (fi->fh == INVALID_HANDLE_VALUE) {
    SERR("CreateFile(<%s>, <O_RDWR>) failed", path);
    success = false;
    goto exit;
  }
  GRN_LOG(ctx, GRN_LOG_INFO, "[io][open] open existing file: <%s>", path);
         ...
         const char *func)
{
  if (grn_fail_malloc_should_fail(length, file, line, func)) {
    ...
    if (version == 0) {
      return grn_mmap_v0(ctx, fi, mi, offset, length);
    } else {
      return grn_mmap_v1(ctx, fi, mi, offset, length);
    }
  }
}
  ...
  grn_ctx *ctx, grn_io *io, fileinfo *fi, grn_io_mapinfo *mi, size_t length)
{
  ...
  if (version == 0) {
    return grn_munmap_v0(ctx, fi, mi, length);
  } else {
    return grn_munmap_v1(ctx, fi, mi, length);
  }
}
...
grn_fileinfo_open(grn_ctx *ctx, fileinfo *fi, const char *path, int flags)
{
  ...
  if (fi->fd == -1) {
    ERRNO_ERR("failed to open file info path: <%s>", path);
    free(fi->path);
    fi->path = NULL;
    return ctx->rc;
  }
  if (fstat(fi->fd, &st) == -1) {
    ERRNO_ERR("failed to stat file info path: <%s>", path);
    grn_close(fi->fd);
    free(fi->path);
    fi->path = NULL;
    return ctx->rc;
         ...
         const char *func)
{
  if (grn_fail_malloc_should_fail(length, file, line, func)) {
    ...
    if (fi) {
      ...
      fd = fi->fd;
      if ((fstat(fd, &s) == -1) ||
          (s.st_size < tail && ftruncate(fd, tail) == -1)) {
        SERR("fstat");
        return NULL;
      }
      flags = MAP_SHARED;
...
#  endif
    ...
    mi->map = mmap(NULL, length, PROT_READ | PROT_WRITE, flags, fd, offset);
    if (mi->map == MAP_FAILED) {
