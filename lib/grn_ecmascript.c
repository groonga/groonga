...
#define assert GRN_ASSERT
  ...
  /* Lemon (SQLite) 3.52.0 or later is required for %extra_context for
   * %realloc and %free. */
  static void *
  realloc_wrap(void *ptr, size_t size, grn_ctx *ctx)
  {
    return GRN_REALLOC(ptr, size);
  }

  static void
  free_wrap(void *ptr, grn_ctx *ctx)
  {
    GRN_FREE(ptr);
  }
...
#undef YYREALLOC
#define YYREALLOC realloc_wrap
#undef YYFREE
#define YYFREE free_wrap
#undef YYDYNSTACK
#define YYDYNSTACK 1
#undef YYSIZELIMIT
#define grn_expr_parserCTX(P) ((P)->ctx )
#define grn_expr_parserCTX_SDECL  grn_ctx *ctx ;
#define grn_expr_parserCTX_PDECL , grn_ctx *ctx 
#define grn_expr_parserCTX_PARAM ,ctx 
#define grn_expr_parserCTX_FETCH  grn_ctx *ctx =yypParser->ctx ;
#define grn_expr_parserCTX_STORE yypParser->ctx =ctx ;
#undef YYERRORSYMBOL
#undef YYERRSYMDT
#undef YYFALLBACK
#define YYNSTATE             166
...
#define YY_MIN_DSTRCTR       83
#define YY_MAX_DSTRCTR       83
/************* End control #defines *******************************************/
...
static int yyGrowStack(yyParser *p){
  ...
  yyStackEntry *pNew;
#ifdef YYSIZELIMIT
  int nLimit = YYSIZELIMIT(grn_expr_parserCTX(p));
#endif
  ...
  if( p->yystack==p->yystk0 ){
    pNew = YYREALLOC(0, newSize*sizeof(pNew[0]), grn_expr_parserCTX(p));
    if( pNew==0 ) return 1;
    memcpy(pNew, p->yystack, oldSize*sizeof(pNew[0]));
  }else{
    pNew = YYREALLOC(p->yystack, newSize*sizeof(pNew[0]), grn_expr_parserCTX(p));
    if( pNew==0 ) return 1;
  }
  p->yystack = pNew;
...
void grn_expr_parserFinalize(void *p){
  ...
  ** element left in the stack */
  ...
  if( pParser->yystack!=pParser->yystk0 ){
    YYFREE(pParser->yystack, grn_expr_parserCTX(pParser));
  }
#endif
}
...
#line 405 "grn_ecmascript.lemon"
{
  grn_expr_append_op(efsi->ctx, efsi->e, GRN_OP_BITWISE_XOR, 2);
}
#line 2377 "grn_ecmascript.c"
        break;
...
#line 2772 "grn_ecmascript.c"
        break;
      case 63: /* unary_expression ::= DECR unary_expression */
#line 686 "grn_ecmascript.lemon"
{
  grn_ctx *ctx = efsi->ctx;
  grn_expr *e = (grn_expr *)(efsi->e);
  grn_expr_dfi *dfi_;
...
#line 794 "grn_ecmascript.lemon"
{
  yylhsminor.yy0 = yymsp[-2].minor.yy0 + 1;
}
#line 2933 "grn_ecmascript.c"
...
#line 833 "grn_ecmascript.lemon"
{
  yylhsminor.yy0 = yymsp[-1].minor.yy0;
}
#line 2985 "grn_ecmascript.c"
