...
{
  if (!o->nrefs) {
    GRN_LOG(grn_log_error, "o->nrefs corrupt");
    return;
  }
  if (o->nrefs < 0xffff) { o->nrefs--; }
  if (PAIRP(o)) { // todo : check cycle
...
E2:
  ...
  // if (!o->nrefs) { GRN_LOG(grn_log_error, "obj->nrefs corrupt"); }
...
opexe(grn_ctx *ctx)
{
  ...
  if (ctx->impl->op == OP_T0LVL || ctx->impl->n_entries > ctx->impl->ncells + GC_THRESHOLD) {
    if (ctx->impl->gc_verbose) {
      ...
      *buf.u.b.curr = '\0';
      GRN_LOG(grn_log_notice, "mgc > ncells=%d envir=<%s>", ctx->impl->n_entries, buf.u.b.head);
      grn_bulk_fin(ctx, &buf);
    }
    grn_ctx_mgc(ctx);
    if (ctx->impl->gc_verbose) {
      GRN_LOG(grn_log_notice, "mgc < ncells=%d", ctx->impl->n_entries);
    }
  ...
  case OP_SDOWN:   /* shutdown */
    GRN_LOG(grn_log_notice, "shutting down..");
  ...
  GRN_LOG(grn_log_error, "illegal op (%d)", ctx->impl->op);
  return NIL;
}
