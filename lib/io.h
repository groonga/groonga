...
#define GRN_IO_SEG_REF(io,segno,addr) {\
  ...
  for (retry = 0;; retry++) {\
	...
	GRN_LOG(grn_log_crit, "deadlock detected! in GRN_IO_SEG_REF(%p, %u, %u)", io, segno, nref);\
        *pnref = 0; /* force reset */ \
	break;\
      }\
      usleep(1000);\
    ...
    if (nref > 10000) {\
      GRN_LOG(grn_log_alert, "strange nref value! in GRN_IO_SEG_REF(%p, %u, %u)", io, segno, nref);\
    }\
    if (!info->map) {\
      if (nref) {\
	GRN_ATOMIC_ADD_EX(pnref, -1, nref);\
	...
	grn_io_seg_map_(ctx, io, segno, info);\
        if (!info->map) {\
          GRN_ATOMIC_ADD_EX(pnref, -1, nref);\
          GRN_LOG(grn_log_crit, "mmap failed!! in GRN_IO_SEG_REF(%p, %u, %u)", io, segno, nref);\
        }\
      }\
    }\
    addr = info->map;\
...
#define GRN_IO_SEG_MAP(io,segno,addr) {\
  grn_io_mapinfo *info = &(io)->maps[segno];\
  uint32_t retry, *pnref = &info->nref;\
  for (retry = 0; !info->map || info->count != grn_gtick; retry++) {\
    ...
    if (nref) {\
      GRN_ATOMIC_ADD_EX(pnref, -1, nref);\
      if (retry >= GRN_IO_MAX_RETRY) {\
        GRN_LOG(grn_log_crit, "deadlock detected!! in GRN_IO_SEG_MAP(%p, %u)", io, segno);\
        break;\
      }\
      usleep(1000);\
    } else {\
      ...
      if (!info->map) {\
        grn_io_seg_map_(ctx, io, segno, info);\
        if (!info->map) {\
          GRN_LOG(grn_log_crit, "mmap failed!! in GRN_IO_SEG_MAP(%p, %u)", io, segno);\
        }\
      }\
      GRN_ATOMIC_ADD_EX(pnref, -1, nref);\
      break;\
