...
#define GRN_IO_SEG_REF(io,segno,addr) {\
  ...
  if (*pnref > 1) { GRN_LOG(&grn_gctx, GRN_LOG_NOTICE, "ref(%d:%d) %d:%s (%x)", segno, *pnref,__LINE__,__FUNCTION__, io->flags); } \
  addr = info->map;\
}
...
#define GRN_IO_SEG_UNREF(io,segno) {\
  if (GRN_IO_EXPIRE_SEGMENT ==\
    ...
    GRN_ATOMIC_ADD_EX(pnref, -1, nref);\
    if (*pnref) { GRN_LOG(&grn_gctx, GRN_LOG_NOTICE, "unref(%d:%d) %d:%s (%x)", segno, *pnref, __LINE__,__FUNCTION__, io->flags); } \
  }\
}
