  ...
  if (!path || strlen(path) <= PATH_MAX - 14) {
    if ((s = GRN_MALLOC(sizeof(grn_db)))) {
      ...
      if ((s->keys = grn_pat_create(ctx, path, GRN_PAT_MAX_KEY_SIZE, 0,
        ...
        ERR(grn_memory_exhausted, "s->keys create failed");
      }
      grn_tiny_array_fin(&s->values);
      GRN_FREE(s);
    } else {
      ERR(grn_memory_exhausted, "grn_db alloc failed");
    }
  } else {
  ...
  if (path && strlen(path) <= PATH_MAX - 14) {
    if ((s = GRN_MALLOC(sizeof(grn_db)))) {
      ...
      if ((s->keys = grn_pat_open(ctx, path))) {
        ...
        ERR(grn_memory_exhausted, "s->keys open failed");
      }
      grn_tiny_array_fin(&s->values);
      GRN_FREE(s);
    } else {
...
grn_rc
grn_db_close(grn_ctx *ctx, grn_db *s)
{
  grn_id id;
  grn_obj **vp;
                ...
                grn_proc_func *init, grn_proc_func *next, grn_proc_func *fin)
{
  ...
  if (name && name_size) {
    ...
    if (!(id = grn_pat_lookup(ctx, s->keys, name, name_size, NULL, &f))) {
      ERR(grn_memory_exhausted, "grn_pat_lookup failed");
      GRN_API_RETURN(NULL);
    }
        ...
        case GRN_SEARCH_TERM_EXTRACT :
          {
            ...
            for (; sp < se; sp += len) {
              ...
              if (!(len = grn_str_charlen_nonnull(sp, se, pat->encoding))) { break; }
            }
          }
          // todo : support op;
          break;
...
_grn_table_key(grn_ctx *ctx, grn_obj *table, grn_id id, uint32_t *key_size)
{
  ...
  switch (table->header.type) {
  case GRN_TABLE_HASH_KEY :
    return _grn_hash_key((grn_hash *)table, id, key_size);
  case GRN_TABLE_PAT_KEY :
    return _grn_pat_key((grn_pat *)table, id, key_size);
  case GRN_TABLE_NO_KEY :
    {
      grn_array *a = (grn_array *)table;
  ...
  if (data) {
    ...
    case GRN_COLUMN_FIX_SIZE :
      if (((grn_ra *)pctx->obj)->header->element_size < value_size) {
        ...
        if (!v) {
          ERR(grn_memory_exhausted, "ra get failed");
...
grn_vector_decode(grn_ctx *ctx, grn_obj *vector, const char *data, uint32_t data_size)
{
  ...
  GRN_B_DEC(v->n_entries, p);
  {
    ...
    if (!offsets) { return grn_memory_exhausted; }
    v->offsets = offsets;
  }
  {
    int i;
               ...
               unsigned int weight, grn_id domain)
{
  ...
  if (!(v->u.v.n_verses & (INITIAL_VERSES_SIZE - 1))) {
    ...
    if (!vp) { return grn_memory_exhausted; }
    v->u.v.verses = vp;
  }
  ...
  if (v->header.flags & GRN_OBJ_DO_SHALLOW_COPY) {
    ...
    if (!(vp->str = GRN_MALLOC(str_len))) { return grn_memory_exhausted; }
...
grn_obj_get_accessor(grn_ctx *ctx, grn_obj *obj, const char *name, unsigned name_size)
{
  ...
  }
  {
    ...
    for (sp = name; (len = grn_str_charlen_nonnull(sp, se, ctx->encoding)); sp += len) {
...
grn_accessor_get_value_(grn_ctx *ctx, grn_accessor *a, grn_id id, uint32_t *size)
{
  ...
  for (;;) {
    ...
    case GRN_ACCESSOR_GET_DB_OBJ :
      value = _grn_pat_key(((grn_db *)ctx->impl->db)->keys, id, size);
      break;
    case GRN_ACCESSOR_LOOKUP :
      /* todo */
      break;
...
grn_obj_close(grn_ctx *ctx, grn_obj *obj)
{
  ...
  if (obj) {
    ...
    case GRN_TYPE :
      ...
      break;
    case GRN_DB :
      rc = grn_db_close(ctx, (grn_db *)obj);
      break;
    case GRN_TABLE_PAT_KEY :
    ...
    case GRN_VECTOR :
      ...
      break;
    case GRN_QUERY :
      rc = grn_query_close((grn_query *)obj);
      break;
    case GRN_COLUMN_VAR_SIZE :
...
grn_db *
grn_db_create(const char *path, int flags, grn_encoding encoding)
{
  return grn_db_create(&grn_gctx, path, encoding);
}

grn_db *
grn_db_open(const char *path)
{
  return grn_db_open(&grn_gctx, path);
}

grn_rc
grn_db_close(grn_db *s)
{
  return grn_db_close(&grn_gctx, (grn_db *)s);
}
