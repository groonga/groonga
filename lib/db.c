...
static bool
grn_db_open_ensure_type(grn_ctx *ctx,
                        grn_db *db,
                        grn_id id,
                        const char *name,
                        ...
                        unsigned int size)
{
  if (grn_table_get(ctx, db->keys, name, name_size) == id) {
    return false;
  }
  ...
  if (grn_table_update_by_id(ctx, db->keys, id, name, name_size) !=
      GRN_SUCCESS) {
    return false;
  }
...
#define GRN_TYPE_SHORT_BINARY_FLAGS    GRN_OBJ_KEY_VAR_SIZE
...
#define GRN_TYPE_BINARY_FLAGS    GRN_OBJ_KEY_VAR_SIZE
...
#define GRN_TYPE_LONG_BINARY_FLAGS    GRN_OBJ_KEY_VAR_SIZE
...
grn_obj_spec_save(grn_ctx *ctx, grn_db_obj *obj)
{
  ...
  /*
   * s->specs is already initialized and does not have the flag information at
   * this point. The flags in obj would be lost in subsequent processing.
   * Therefore, if obj is a column, copy its flags to s->specs here.
   */
  if (grn_obj_is_column(ctx, (grn_obj *)obj)) {
    grn_column_flags flags = grn_ja_get_flags(ctx, (grn_ja *)obj);
    s->specs->header->flags |= flags;
  }
...
grn_db_init_builtin_types(grn_ctx *ctx)
{
  ...
  obj =
    deftype(ctx, GRN_TYPE_JSON_NAME, GRN_TYPE_JSON_FLAGS, GRN_TYPE_JSON_SIZE);
