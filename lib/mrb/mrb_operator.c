...
grn_mrb_value_from_operator(mrb_state *mrb, grn_operator op)
{
  ...
  mrb_op_raw = mrb_int_value(mrb, op);
...
#define DEFINE_OPERATOR(name)                                   \
  mrb_funcall(mrb, klass_obj, "register", 1,                    \
              mrb_funcall(mrb, klass_obj, "new", 2,             \
                          ...
                          mrb_int_value(mrb, GRN_OP_ ## name)))
  ...
  DEFINE_OPERATOR(NEAR_NO_OFFSET);
  ...
  DEFINE_OPERATOR(ORDERED_NEAR_PHRASE);
  DEFINE_OPERATOR(NEAR_PHRASE_PRODUCT);
  DEFINE_OPERATOR(ORDERED_NEAR_PHRASE_PRODUCT);
