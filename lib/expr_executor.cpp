...
namespace {
   ...
   * - uint16_t -> int32_t
   * - uint32_t -> int64_t
   * - uint64_t -> int64_t (When X does not over INT64_MAX + 1)
   * - uint64_t -> double (When X over INT64_MAX + 1)
   */
                                                    ...
                                                    grn_obj *result)
  {
    if (y == -1) {
      ...
      grn_obj_reinit(ctx, result, GRN_DB_INT16, 0);
      grn::bulk::set<int16_t>(ctx, result, static_cast<int16_t>(x) / y);
    } else {
      grn::bulk::set<RESULT_TYPE>(ctx, result, static_cast<RESULT_TYPE>(x / y));
    }
    return true;
                                                    ...
                                                    grn_obj *result)
  {
    if (y == -1) {
      ...
      grn_obj_reinit(ctx, result, GRN_DB_INT64, 0);
      grn::bulk::set<int64_t>(ctx, result, static_cast<int64_t>(x) / y);
    } else {
      grn::bulk::set<RESULT_TYPE>(ctx, result, static_cast<RESULT_TYPE>(x / y));
    }
    return true;
    ...
    case GRN_OP_NOT_EQUAL:
      {
        ...
        grn_obj *y = NULL;
        CHECK(pop2alloc1(ctx, data, x, y, data.res));
        bool is_not_equal = grn_operator_exec_not_equal(ctx, x, y);
        grn_obj_reinit(ctx, data.res, GRN_DB_BOOL, 0);
    ...
    case GRN_OP_LESS:
      {
        ...
        grn_obj *y = NULL;
        CHECK(pop2alloc1(ctx, data, x, y, data.res));
        bool r = grn_operator_exec_less(ctx, x, y);
        grn_obj_reinit(ctx, data.res, GRN_DB_BOOL, 0);
    ...
    case GRN_OP_LESS_EQUAL:
      {
        ...
        grn_obj *y = NULL;
        CHECK(pop2alloc1(ctx, data, x, y, data.res));
        bool r = grn_operator_exec_less_equal(ctx, x, y);
        grn_obj_reinit(ctx, data.res, GRN_DB_BOOL, 0);
        GRN_BOOL_SET(ctx, data.res, r);
      }
      data.code++;
