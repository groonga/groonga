...
namespace grn::dat {
  ...
  void
  FileImpl::create(const char *path, UInt64 size)
  {
  ...
  FileImpl::create_(const char *path, UInt64 size)
  {
    if ((path != NULL) && (path[0] != '\0')) {
      ...
      const DWORD file_pos =
        ::SetFilePointer(file_, size_low, &size_high, FILE_BEGIN);
      GRN_DAT_THROW_IF(IO_ERROR,
                       (file_pos == INVALID_SET_FILE_POINTER) &&
                         (::GetLastError() != 0));
