...
#define GRN_TINY_ARRAY_EACH_BEGIN(array, head, tail, value)                    \
  do {                                                                         \
    int _block_id;                                                             \
    const grn_id _head = (head);                                               \
    const grn_id _tail = (tail);                                               \
    ...
    for (_block_id = 0;                                                        \
      ...
      if (_block) {                                                            \
        int _i;                                                                \
        for (_i = 0; _i < _n_elements && _current <= _tail;                    \
             _i++, _current++) {                                               \
...
struct _grn_table_sort_optarg {
  ...
  int (*compar)(grn_ctx *ctx,
                grn_obj *table1,
                void *target1,
                unsigned int target1_size,
                grn_obj *table2,
                void *target2,
                unsigned int target2_size,
                void *compare_arg);
  void *compar_arg;
  grn_obj *proc;
  int offset;
...
int
grn_rec_unit_size(grn_rec_unit unit, int rec_size);
